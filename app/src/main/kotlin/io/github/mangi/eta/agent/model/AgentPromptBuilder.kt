        ...
        messages.put(
            systemMessage(
                (if (!config.supportsVision && ModelFeaturePreferences.visionEnabled()) {
                    ...
                    "协作工具可用且任务能独立拆分时，默认优先并行委派多个职责与任务分工匹配的空闲子代理，不要等一个完成才启动另一个；" +
                    "按模块或互不重叠的文件划分职责，主代理同时推进集成与验证；有数据依赖、同文件写冲突或必须基于成品的审查则保持顺序。" +
                    "不要为凑并发拆分琐碎任务、重复付费请求或把任务全排到同一个代理；派发成功不代表工作完成，必须读取结果、检查差异并验证。" +
