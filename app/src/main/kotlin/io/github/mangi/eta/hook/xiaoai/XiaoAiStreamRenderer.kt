...
internal class XiaoAiStreamRenderer(
    ...
    fun onEvent(event: AgentEvent) {
        when (event) {
            ...
            is AgentEvent.ProviderResponseStarted -> render(
                text(R.string.injected_reasoning, "Armilla is reasoning…"),
            )
            ...
            is AgentEvent.ToolStarted -> {
                if (synchronized(streamedText) { streamedText.isEmpty() }) {
                    render(text(R.string.injected_executing, "Armilla is working…"))
                }
            }
    ...
    fun complete(content: String) {
        ...
        val finalText = content.trim().ifBlank {
            text(R.string.injected_completed, "Armilla completed this task")
        }
    ...
    fun fail() {
        ...
        render(
            text(
                ...
                "Armilla could not complete the task. Try again later",
            ),
            immediate = true,
        )
    }
