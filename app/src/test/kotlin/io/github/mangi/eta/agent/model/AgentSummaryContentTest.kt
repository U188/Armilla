...
class AgentSummaryContentTest {
    @Test fun escapedLargeJpegIsNotCountedAsTextAndSourceIsUnchanged() {
        ...
        val content = JSONArray().put(JSONObject().put("type", "text").put("text", "继续检查截图"))
        ...
        assertTrue(projected.contains("继续检查截图"))
    ...
    @Test fun nestedToolMediaIsProjectedButTextAndIdentitySurvive() {
        val source = """[{"type":"tool-result","id":"call-a","content":[{"type":"text","text":"已完成第一步"},{"type":"image","source":{"type":"base64","data":"large-private-image"}},{"type":"input_audio","data":"audio"}]}]"""
        val output = JSONArray(AgentSummaryContent.project(source)).getJSONObject(0)
        assertEquals("call-a", output.getString("id"))
        assertEquals("已完成第一步", output.getJSONArray("content").getJSONObject(0).getString("text"))
